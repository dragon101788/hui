...
int list_add_prior_delete_next(page_list &L,int w,int h)
{
	...
	//debug("list_add_prior_delete_next: i=%d;j=%d!!!\n",i,j);
	page_list newpage=new page;//(page_list)malloc(sizeof(page));
        newpage->img.SetBuffer(w,h);
	head->prior=newpage;
	newpage->next=head;
...
public:
	...
	//	if(const_page!=1)	
	//	{
	//	debug("doTimer cache_up=%d cache_down=%d!!!!!!!!!!!!!!!!!!!!\n",cache_up,cache_down);
	...
	void doRender()
	{
		...
		if (py<0)
		{
			...
			//	{
				...
				{
					while(cache_up) debug("wait for cache_up clean!!!!!!1\n");//等待上一次的缓存完成
					cache_up=1;
					//debug("start cache up\n!!!!!\n");
					flag_set_timer=1;
				//	TimerSet(0);
				}
		...
		else  if(py>0)
		 {
			...
			//	page++;
				...
				else
				{
					...
					{
						while(cache_down) debug("wait for cache_down clean!!!!!!!\n");
						cache_down=1;
						//debug("start cache_down!!!!!!!!\n");
						//TimerSet(0);
						flag_set_timer=1;
					}
				}
		...
		debug("page=%d,const_page=%d,print_num=%d,txt_len=%d,sy=%d,py=%d!!\n",page,const_page,print_num,txt_len,sy,py);
